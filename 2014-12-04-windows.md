...
* Grep for Windows - GnuWin32 - 2.5.4  : for sjis file(基本的にmsys2のを使うため、パス通さずaliasでgrepsjisとかにしとく). 依存ライブラリ(grep-2-5-x-dep.zip)が必要. Ref. <http : //gnuwin32.sourceforge.net/packages/grep.htm>
* MSYS2
...
* Workaround for openssh
	...
	mkdir /home
	<!--- cmd //c "mklink /D D:\\admin\\Tools\\msys32\\home\\admin C:\\Users\\admin" 2>&1 | nkf32.exe -w --->
...
※参考 [mintty-color-schemes/base16-tomorrow.minttyrc at master · oumu/mintty-color-schemes · GitHub](https://github.com/oumu/mintty-color-schemes/blob/master/base16-mintty/base16-tomorrow.minttyrc)
...
* コマンドプロンプトを起動(別アプリとして)
		start
* .ssh,.m2ディレクトリはC:\Users\adminにおく(それが一番楽)
