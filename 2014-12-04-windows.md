...
* Workaround for git-prompt, git-completion
	`git`を`winpty git`にaliasしてるので、
	git-prompt.shのgitコマンドをすべて\gitにエスケープが必要っぽいTODO いいやりかたないか
...
##### Install with pacman
	...
	pacman -S mingw-w64-i686-nodejs # For npm
	pacman -S mingw-w64-i686-ruby # For gem
	pacman -S p7zip
...
##### Install with gem
	...
	gem install mdl -r -p {proxy url}
	gem install yaml-lint -r -p {proxy url}
